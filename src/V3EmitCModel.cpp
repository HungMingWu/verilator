...
class EmitCModel final : public EmitCFunc {
    ...
    void emitHeader(AstNodeModule* modp) {
        ...
        if (v3Global.opt.metadata()) {
            ...
            puts("    static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code);\n");
            puts("public:\n");
            puts("    /// Trace signals in the model; called by application code\n");
    ...
    void emitMeta(AstNodeModule* modp) {
        ...
        puts("template <typename T>\n");
        puts("VL_ATTR_COLD\n");
        puts("void MetaStore<T>::trace(VerilatedVcdC * tfp, int levels, int options)\n");
        puts("{\n");
        puts("    if (false && levels && options) {}  // Prevent unused\n");
        puts("    printf(\"register tracing function, this = %p\\n\", this);\n");
        puts("    tfp->spTrace()->addInitCb(&MetaStore::trace_init, this);\n");
        puts("    tfp->spTrace()->addFullCb(&trace_full, this);\n");
        puts("    tfp->spTrace()->addChgCb(&trace_chg, this);\n");
        puts("    tfp->spTrace()->addCleanupCb(&trace_cleanup, this);\n");
        puts("}\n");
        puts("template <typename T>\n");
        puts("VL_ATTR_COLD\n");
        ...
        puts("    tracep->declBus(c + 14, \"out_small\", false, -1, 1, 0);\n");
        puts("    tracep->declQuad(c + 15, \"out_quad\", false, -1, 39, 0);\n");
        puts("    tracep->declArray(c + 17, \"out_wide\", false, -1, 69, 0);\n");
        puts("    tracep->declBus(c + 20, \"in_small\", false, -1, 1, 0);\n");
        puts("    tracep->declQuad(c + 21, \"in_quad\", false, -1, 39, 0);\n");
        puts("    tracep->declArray(c + 23, \"in_wide\", false, -1, 69, 0);\n");
        puts("    tracep->pushNamePrefix(\"top \");\n");
        puts("    tracep->declBit(c + 12, \"clk\", false, -1);\n");
        puts("    tracep->declBit(c + 13, \"reset_l\", false, -1);\n");
        puts("    tracep->declBus(c + 14, \"out_small\", false, -1, 1, 0);\n");
        puts("    tracep->declQuad(c + 15, \"out_quad\", false, -1, 39, 0);\n");
        ...
        puts("    tracep->declQuad(c + 21, \"in_quad\", false, -1, 39, 0);\n");
        puts("    tracep->declArray(c + 23, \"in_wide\", false, -1, 69, 0);\n");
        puts("    tracep->pushNamePrefix(\"sub \");\n");
        puts("    tracep->declBit(c + 12, \"clk\", false, -1);\n");
        puts("    tracep->declBit(c + 13, \"reset_l\", false, -1);\n");
        puts("    tracep->declBus(c + 1, \"count_c\", false, -1, 31, 0);\n");
        puts("    tracep->declArray(c + 2, \"mem\", false, -1, 310, 0);\n");
        puts("    tracep->popNamePrefix(2);\n");
        puts("}\n");
        puts("template <typename T>\n");
        puts("VL_ATTR_COLD\n");
        puts("void MetaStore<T>::trace_full(void* voidSelf, VerilatedVcd * tracep)\n");
        puts("{\n");
        ...
        puts("    tracep->fullBit(oldp + 13, (rootp->reset_l));\n");
        puts("    tracep->fullCData(oldp + 14, (rootp->out_small), 2);\n");
        puts("    tracep->fullQData(oldp + 15, (rootp->out_quad), 40);\n");
        puts("    tracep->fullWData(oldp + 17, (rootp->out_wide), 70);\n");
        puts("    tracep->fullCData(oldp + 20, (rootp->in_small), 2);\n");
        puts("    tracep->fullQData(oldp + 21, (rootp->in_quad), 40);\n");
        puts("    tracep->fullWData(oldp + 23, (rootp->in_wide), 70);\n");
        puts("}\n");
        puts("template <typename T>\n");
        puts("VL_ATTR_COLD\n");
        ...
        puts("template <typename T>\n");
        puts("VL_ATTR_COLD\n");
        puts("void MetaStore<T>::trace_cleanup(void* voidSelf, VerilatedVcd * tracep)\n");
        puts("{\n");
        puts("    MetaStore<T>* const __restrict vlSelf VL_ATTR_UNUSED = static_cast<MetaStore<T> *>(voidSelf);\n");
        puts("    auto * const __restrict rootp = vlSelf->rootp;\n");
        puts("    auto * const __restrict vlSymsp VL_ATTR_UNUSED = rootp->vlSymsp;\n");
        ...
        puts("{\n");
        puts("    // Callback from tracep->open()\n");
        puts("    MetaStore<T>* const __restrict vlSelf VL_ATTR_UNUSED = static_cast<MetaStore<T> *>(voidSelf);\n");
        puts("    auto * const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->rootp->vlSymsp;\n");
        ...
        puts("    vlSelf->trace_init_top(tracep);\n");
