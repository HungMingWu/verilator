...
class EmitVBaseVisitor1 VL_NOT_FINAL : public EmitCBaseVisitor {
    // MEMBERS
...
public:
    ...
    explicit EmitVBaseVisitor1(bool suppressUnknown, AstSenTree* domainp)
        : m_suppressUnknown{suppressUnknown}
        , m_sensesp{domainp} {}
    virtual ~EmitVBaseVisitor1() override = default;
};

//######################################################################
// Emit to an output file

class EmitVFileVisitor1 final : public EmitVBaseVisitor1 {
    // MEMBERS
    V3OutFile* m_ofp;
    // METHODS
...
public:
    EmitVFileVisitor1(AstNode* nodep, V3OutFile* ofp, bool trackText, bool suppressUnknown)
        : EmitVBaseVisitor1{suppressUnknown, nullptr} {
        m_ofp = ofp;
        m_trackText = trackText;
        iterate(nodep);
    }
    virtual ~EmitVFileVisitor1() override = default;
};

//######################################################################
// Emit to a stream (perhaps stringstream)

class EmitVStreamVisitor1 final : public EmitVBaseVisitor1 {
    // MEMBERS
    std::ostream& m_os;
    // METHODS
...
public:
    EmitVStreamVisitor1(const AstNode* nodep, std::ostream& os)
        : EmitVBaseVisitor1{false, nullptr}
        , m_os(os) {  // Need () or GCC 4.8 false warning
        iterate(const_cast<AstNode*>(nodep));
    }
    virtual ~EmitVStreamVisitor1() override = default;
};

//######################################################################
// Emit to a stream (perhaps stringstream)

class EmitVPrefixedFormatter1 final : public V3OutFormatter {
    std::ostream& m_os;
...
public:
    ...
    EmitVPrefixedFormatter1(std::ostream& os, const string& prefix, int flWidth)
    ...
    virtual ~EmitVPrefixedFormatter1() override {
        if (m_column) puts("\n");
    }
};

class EmitVPrefixedVisitor1 final : public EmitVBaseVisitor1 {
    // MEMBERS
    EmitVPrefixedFormatter1 m_formatter;  // Special verilog formatter (Way down the
...
public:
    EmitVPrefixedVisitor1(const AstNode* nodep, std::ostream& os, const string& prefix, int flWidth,
                         AstSenTree* domainp, bool user3mark)
        : EmitVBaseVisitor1{false, domainp}
        , m_formatter{os, prefix, flWidth} {
        if (user3mark) VNUser3InUse::check();
        iterate(const_cast<AstNode*>(nodep));
    }
    virtual ~EmitVPrefixedVisitor1() override = default;
};

//######################################################################
// EmitV class functions

void V3EmitV1::verilogForTree(const AstNode* nodep, std::ostream& os) {
    { EmitVStreamVisitor1{nodep, os}; }
}

void V3EmitV1::verilogPrefixedTree(const AstNode* nodep, std::ostream& os, const string& prefix,
                                  int flWidth, AstSenTree* domainp, bool user3mark) {
    { EmitVPrefixedVisitor1{nodep, os, prefix, flWidth, domainp, user3mark}; }
}

void V3EmitV1::emitvFiles() {
    ...
    for (AstNodeFile* filep = v3Global.rootp()->filesp(); filep;
        ...
        if (vfilep && vfilep->tblockp()) {
            ...
            { EmitVFileVisitor1{vfilep->tblockp(), &of, true, false}; }
        }
    }
}

void V3EmitV1::debugEmitV(const string& filename) {
    UINFO(2, __FUNCTION__ << ": " << endl);
    V3OutVFile of(filename);
    { EmitVFileVisitor1{v3Global.rootp(), &of, true, true}; }
}
