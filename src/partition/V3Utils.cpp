...
class ExpandVisitor2 final : public VNVisitor {
private:
    // METHODS
    VL_DEBUG_FUNC;  // Declare debug()
    ...
    virtual void visit(AstExtend* nodep) override {
        ...
        int extendWidth = dtypep->widthMin() - lhsp->dtypep()->widthMin();
        if (extendWidth <= 0) return;
        lhsp->unlinkFrBack();
        AstNode* constp = new AstConst(nullptr, AstConst::WidthedValue{}, extendWidth, 0);
        AstNode* newp = new AstConcat(nullptr, constp, lhsp);
        nodep->replaceWith(newp);
        VL_DO_DANGLING(nodep->deleteTree(), nodep);
    }
    virtual void visit(AstSel* nodep) override { 
        ...
        if (VN_IS(fromp, Extend) ||
            ...
            if (VN_IS(nodep->lsbp(), Const)) {
                if (nodep->widthp()->isOne()) {
                    if (VN_IS(nodep->lsbp(), Const)) {
                        uint32_t d = static_cast<uint32_t>(VN_AS(nodep->lsbp(), Const)->toSInt());
                        newp = new AstShiftR{nullptr, fromp, new AstConst{nullptr, d}};
                    } else {
                        AstNode* lsbp = nodep->lsbp();
                        lsbp->unlinkFrBack();
                        newp = new AstShiftR{nullptr, fromp, lsbp};
                    }
                } else {
                    uint32_t r = static_cast<uint32_t>(VN_AS(nodep->lsbp(), Const)->toSInt() +
                        VN_AS(nodep->widthp(), Const)->toSInt() - 1);
                    uint32_t l = static_cast<uint32_t>(VN_AS(nodep->lsbp(), Const)->toSInt());
                    AstNode* shiftrp = new AstShiftR{nullptr, fromp, new AstConst{nullptr, l}};
                    V3Number num(static_cast<AstNode*>(nullptr), r - l + 1);
                    num.setAllBits1();
                    AstNode* constp = new AstConst(nullptr, num);
                    newp = new AstAnd(nullptr, constp, shiftrp);
                }
            } else {
                AstNode* lsbp = nodep->lsbp();
                lsbp->unlinkFrBack();
                AstNode* shiftrp = new AstShiftR{nullptr, fromp, lsbp};
                AstNode* constp = new AstConst(nullptr, AstConst::WidthedValue{},
                    nodep->widthConst(), (1 << nodep->widthConst()) - 1);
                newp = new AstAnd(nullptr, constp, shiftrp);
            }
            nodep->replaceWith(newp);
            VL_DO_DANGLING(nodep->deleteTree(), nodep);
    ...
    virtual void visit(AstAssignDly* nodep) override { 
        iterateChildren(nodep);
        printf("Before visit: AstAssignDly\n");
        V3EmitV::verilogForTree(nodep, std::cout);
        printf("\nAfter visit AstAssignDly:\n");

    }

public:
    // CONSTRUCTORS
    explicit ExpandVisitor2(AstNodeModule* nodep) { iterate(nodep); }
    virtual ~ExpandVisitor2() override = default;
};
